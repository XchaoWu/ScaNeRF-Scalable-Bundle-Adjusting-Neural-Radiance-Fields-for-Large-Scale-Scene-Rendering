    ...
    float &output_alpha)
{
    ...
    //                 float3 level_grid_size = block_size / make_float3(level_resolution - 1);
    //                 int3 bottom_left_idx =  make_int3((pts-block_corner)/level_grid_size);
    //                 float3 voxel_min_vertex = make_float3(bottom_left_idx) * level_grid_size + block_corner;
    //                 float3 offset = (pts - voxel_min_vertex) / level_grid_size;
    //                 float weight[8];
